...
class RemoteTestProxy:
    ...
    def load(self, loader):
        ...
        loader.add_option(name = "dbPort", typespec = int, default= 3306, help = "Provide port for the cache db, use dbPort")
    ...
    def request(self, flow: http.HTTPFlow) -> None:
        ...
        init_url_file = open('/var/www/html/init_url', 'r')
        service_type_file = open('/var/www/html/deadcode', 'r')
        init_url = init_url_file.read()
        service_type = service_type_file.read()
        init_url_file.close()
        service_type_file.close()
        flow.request.headers.add("initiating-url", init_url)
        
        try:
            with self.connection.cursor() as cursor:
                requestUrl = flow.request.pretty_url.split("?")[0]
                flow.initiatingUrl = self.parseUrl(init_url)
        ...
        if not sql_response:
            ...
            temp_content = None
            if service_type == "dce" and sql_response[1] != None:
